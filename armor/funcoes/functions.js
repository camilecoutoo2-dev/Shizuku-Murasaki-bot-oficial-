...
/**
 * Envia arquivos para a Shizuku Uploads via Bot/API
 * @param {Buffer} buffer - O conteúdo do arquivo
 * @param {String} name - Nome do arquivo com extensão (ex: 'video.mp4')
 * @param {String} token - O Token Shizuku do usuário, se você não tiver, crie a sua conta em: https://upload.shizuku-apis.online/ e pegue seu token 
 */
 
const token = "Kitty_katty";
async function JpUpload(buffer, name, mimetype) {
    try {
        const form = new FormData();
        form.append('file', buffer, {
            filename: name || 'shizuku_file.png',
            contentType: mimetype || 'application/octet-stream' 
        });

        const response = await axios.post('https://shizukuapis.space/api/upload', form, {
            headers: {
               ...form.getHeaders(),
               'token-upload-shizuku': token
            },
        ...
        if (response.data.link) {
            ...
            return "Servidor não retornou link";
        }
    } catch (error) {
        console.error("Erro Shizuku uploads:", error.response?.status, error.response?.data || error.message);
        if (error.response?.status === 413) return "Arquivo maior que 100MB no servidor";
        if (error.response?.status === 403) return "Token inválido";
        return null;
    }
}
...
const banner2 = cfonts.render(('Criada por: jpzinh 👑|WhatsApp: +55 93 9242-0001'), {
font: 'console',
align: 'center',
gradrient: [`${cor4}`,`${cor2}`], 
...
lineHeight: 1
 ...
 const banner3 = cfonts.render((`SHIZUKU-BOT`), {
font: 'slick',             
align: 'center',           
...
const authorname = "Shizuku-Bot|by: jpzinh"
const packname = "créditos da base: Vitor Gabriel"

const chyt = "559391423261@s.whatsapp.net";
const nit = "559392420001@s.whatsapp.net";
const supre = "559391423261@s.whatsapp.net";
...
module.exports = { getBuffer, fetchJson, fetchText, generateMessageID, getGroupAdmins, getMembros, getRandom, banner2, temporizador, color, recognize, bgcolor, isFiltered, addFilter, banner3, chyt, getExtension, convertSticker, upload, nit, getpc, supre, JpUpload }
